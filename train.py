...
def train(args):
    ...
    with tf.Session(graph=tf_graph) as sess:
        with sess.as_default():
            ...
            for d in {2}: #range(2,5):
                log_count = '/home/siri0005/Documents/multimodaltraj_2/log/g2k_lstm_mc_counts_{0}.txt'.format(d)
                log_count_f = open(log_count, 'w')
                log_dir = '/home/siri0005/Documents/multimodaltraj_2/log/g2k_lstm_mc_error_log_{0}.csv'.format(d)
                log_dir_fde = '/home/siri0005/Documents/multimodaltraj_2/log/g2k_lstm_mc_fde_log_{0}.csv'.format(d)
                ...
                while e < args.num_epochs:
                    ...
                    if e == 0:
                        ...
                        krnl_mdl = mc.g2k_lstm_mc(in_features=nghood_enc.input, out_size=batch_v.shape[1],
                    ...
                    for b in range(dataloader.num_batches): # range(2):
                        ...
                        tf.initialize_variables(var_list=[krnl_mdl.bias_v, krnl_mdl.weight_c, krnl_mdl.weight_o, krnl_mdl.weight_v]).run()
                        sess.run(fetches=tf.initialize_all_variables())
                    ...
                    ade = np.stack(euc_loss)
                    np.savetxt(fname=log_dir, X=np.reshape(ade, newshape=(ade.shape[0] * ade.shape[1], ade.shape[2])),
                               delimiter=",")
                    np.savetxt(fname=log_dir_fde, X=np.stack(fde), delimiter=",")


                    # log_f.write('{0}'.format(euc_loss))
                    if (e * dataloader.num_batches + b) % args.save_every == 0 and ((e * dataloader.num_batches + b) > 0):
                        checkpoint_path = os.path.join('/home/serene/PycharmProjects/multimodaltraj_2/save', 'g2k_mc_model_val_{0}.ckpt'.format(b))
            ...
            for vb in range(dataloader.valid_num_batches):
                ...
                checkpoint_path = os.path.join('/home/siri0005/Documents/multimodaltraj_2/log/save',
