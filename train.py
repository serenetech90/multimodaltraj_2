...
def train(args):
    ...
    #             log_count = '/home/serene/PycharmProjects/multimodaltraj_2/log/g2k_lstm_counts_{0}.txt'.format(
    #                 d)
    #             log_count_f = open(log_count, 'w')
    #             log_dir = '/home/serene/PycharmProjects/multimodaltraj_2/log/g2k_mp_error_log_kfold_{0}.csv'.format(
    #                 d)
    #             log_dir_fde = '/home/serene/PycharmProjects/multimodaltraj_2/log/g2k_mp_fde_log_kfold_{0}.csv'.format(
    #                 d)
    #
    ...
    #                         # map = []
    #                         # outer_def = sess.graph_def
    #                         # for i in sess.graph.get_all_collection_keys():
    #                         #     map.append(sess.graph.get_collection(name=i))
    ...
    #                                                            'g2k_mp_model_kfold_train_{1}_{0}_{2}.ckpt'.format(e, d, b))
    #
    #                             # uninitialized_vars = []
    ...
    #                             # with out_sess.as_default():
    #                             # keys = list(sess.graph.get_all_collection_keys())
    #                             # for k, v in zip(keys, map):
    #                             #     for e in v:
    #                             #         tf.import_graph_def(graph_def=outer_def,
    #                             #                             input_map={"weight_input/weight_i/read": e})
    #
    #                             try:
    #                                 sess.run(fetches=tf.initialize_all_variables()) #critical when it initializes all variables stored in default graph each time
    #                             except tf.errors.FailedPreconditionError:
    ...
    #                     sess.close()
    #
    #                 # tf.get_default_graph().clear_collection(name='variables')
    #                 # tf.get_default_graph().clear_collection(name='trainable_variables')
    #                 # tf.reset_default_graph()
    #                 # del out_sess
    #                 # out_sess.graph._nodes_by_name = {}
    #                 # out_sess.graph._nodes_by_id = {}
    #
    ...
    with tf.Session(graph=tf_graph) as val_sess:
        graph = nx_g.online_graph(args)
        # out_sess.close()
        ...
        checkpoint_path = '/home/serene/PycharmProjects/multimodaltraj_2/save'
        ...
        for vb in range(dataloader.valid_num_batches):
            ...
            tf.initialize_variables(var_list=[krnl_mdl.weight_r, krnl_mdl.weight_v, krnl_mdl.bias_v]).run()  # , krnl_mdl.embed_vis
            tf.initialize_variables(var_list=[krnl_mdl.cost, krnl_mdl.attn,  krnl_mdl.weight_c, krnl_mdl.weight_o]).run()  #krnl_mdl.attn,
            ...
            for frame in batch:
                ...
                # hidden_state,
                pred_path, prob_mat = \
                    val_sess.run([krnl_mdl.pred_path_band,  krnl_mdl.cost],  #krnl_mdl.hidden_states,
                             feed_dict={
                                 krnl_mdl.outputs: #st_embeddings,
                                 np.concatenate((st_embeddings, vislet_emb.eval()), axis=0),
                                 krnl_mdl.ngh: reg_ng,
