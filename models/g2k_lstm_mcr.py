...
class g2k_lstm_mcr():
    def __init__(self, in_features, out_size):
        ...
        self.outputs = tf.placeholder(dtype=tf.float64,
                                      shape=[in_features.shape[0],in_features.shape[0]], name="outputs")
        ...
        self.weight_k = tf.Variable(name='weight_k', initial_value= \
            self.init_w(shape=(8, in_features.shape[0])),
    ...
    def forward(self, outputs, ngh, visual_path):
        ...
        embedded_spatial_vislet = self.weight_v * self.visual_path + self.bias_v
        ...
        self.cost = tf.matmul(self.cost, tf.ones(shape=(self.outputs.shape[0], 16),
                                                 dtype=tf.float64))

        self.pred_path_band = tf.reshape(self.cost, (2, 8, self.outputs.shape[0]))
