...
class g2k_lstm_mcr():
    def __init__(self, in_features,rel_features, hidden_states, hidden_size, obs_len, num_nodes, lambda_reg):
        ...
        with tf.variable_scope("krnl_weights"):
            ...
            self.weight_c = tf.Variable(name='weight_c', initial_value= \
                                        ...
                                        # shape=tf.shape(1,in_features.shape[1].value),
                                        dtype=tf.float64)
        with tf.variable_scope('krnl_embed'):
    ...
    def forward(self):
        ...
        # else:
        self.cost = tf.Variable(tf.matmul(tf.Variable(tf.matmul(self.weight_v, self.outputs) + self.bias_v),
                                          self.ngh))
